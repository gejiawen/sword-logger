...
function detectLogFilePath() {
    ...
    if (this.config.logSlice) {
        fileName = this.config.logFilePrefix + '-' + this.section + this.config.logFileSuffix
    } else {
        fileName = this.config.logFilePrefix + this.config.logFileSuffix
    }
...
function saveRecord(record) {
    ...
    if (instance.config.logSlice && (instance.section !== getCurrentDateString())) {
...
function commonModel(opts) {
    instance  = new Logger(opts)
    return instance
}

module.exports = function (opts) {
    var merged = _.extend(default_options, opts)

    return merged.middlewareModel ? middlewareModel(opts) : commonModel(opts)
}
