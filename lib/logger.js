...
function saveRecord(record) {
    ...
    if (_.includes(['template', 'request', 'response', 'render'], category)) {
        fields[field] = value
...
function renderSerializeSave(fields) {
    ...
    instance.bunyan[parseLevel(type)](obj, fmtRenderMsg())
}

function parseLevel(status, duration) {
    if (status >= 400) {
        return 'error'
    } else if (typeof status === 'string' && _.includes(['TIMEOUT', 'ERROR'], status.toUpperCase())) {
        return 'error'
    } else {
...
module.exports = function (opts) {
    ...
    return function *(next) {
        ...
        // here is temporarily solution
        // render（模板渲染） & interaction（页面交互）
